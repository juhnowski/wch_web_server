...
0x57, 0xAB,
01, 02, 03, 04, 05, 06, 192, 168, 1, 105, 255, 255, 255, 0, 192, 168, 1, 1};
...
0x57, 0xAB,
MODE_TCPCLIENT, 1000 / 256, 1000 % 256, 192, 168, 1, 105, 1000 / 256, 1000 % 256 };
...
const char Json_main[]= {"{\"tag_1\":1,\"tag_2\":2,\"tag_3\":3}"};
...
void ParseURLType(char *type, char * buf)
{
    if (strstr(buf, ".html") || strstr(name, "HTTP")) /* html type */
        ...
        *type = PTYPE_GIF;
    else if (strstr(buf, ".json"))                    /* json type */
        *type = PTYPE_JSON;
    else
        *type = PTYPE_ERR;
}
    ...
    while(reqnum){
        ...
        switch (http_request.METHOD)
        {
            ...
            case METHOD_GET:                                        //'get' request
                ...
                else if(strstr(name, "test") != NULL) {             //Request to get the "main" web page
                    pagelen = strlen(Json_main);
                    copy_flash(Json_main, pagelen);
                }
